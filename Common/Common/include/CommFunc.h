	...
	default:
		{
			...
			switch ( l_error_code )
			{
			case ERR_MC_BASE:return strcat(strcpy(l_buffer,itoa(error_code,l_convt,10)),RES_STRING(CO_COMMFUNC_H_00071)); //"(GameServer/GateServer->Client返回的错误码空间1－500)");
			case ERR_PT_BASE: return strcat(strcpy(l_buffer,itoa(error_code,l_convt,10)),RES_STRING(CO_COMMFUNC_H_00072)); //"(GroupServer->GateServer返回的错误码空间501－1000)"); 
			case ERR_AP_BASE: return strcat(strcpy(l_buffer,itoa(error_code,l_convt,10)),RES_STRING(CO_COMMFUNC_H_00073)); //"(AccountServer->GroupServer返回的错误码空间1001－1500)");
			case ERR_MT_BASE: return strcat(strcpy(l_buffer,itoa(error_code,l_convt,10)),RES_STRING(CO_COMMFUNC_H_00074)); //"(GameServer->GateServer返回的错误码空间1501－2000)");
			default:return strcat(strcpy(l_buffer,itoa(error_code,l_convt,10)),RES_STRING(CO_COMMFUNC_H_00075)); //"(Jack太BT了，弄个错误连我都不认识。)");
			}
		}
	}
}
