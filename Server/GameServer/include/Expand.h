...
#ifdef defPARSE_LOG
	char	szPrint[256] = "";
	//sprintf(szPrint, "取角色%s, %d号属性成功。值：%d\n", pCCha->GetLogName(), sAttrIndex, nAttrVal);
	sprintf(szPrint, RES_STRING(GM_EXPAND_H_00005), pCCha->GetLogName(), sAttrIndex, nAttrVal);
...
if (bSuccess)
{
	...
	char	szValue[128] = "";
	if (lSetRet == 1)
		//sprintf(szValue, "成功，但请求的值超过最大值。设置值：%d，现在的值：%d。", lValue, pCCha->getAttr(sAttrIndex));
		sprintf(szValue, RES_STRING(GM_EXPAND_H_00008), lValue, pCCha->getAttr(sAttrIndex));
	else
		//sprintf(szValue, "成功。设置值：%d，现在的值：%d。", lValue, pCCha->getAttr(sAttrIndex));
		sprintf(szValue, RES_STRING(GM_EXPAND_H_00009), lValue, pCCha->getAttr(sAttrIndex));
	//sprintf(szPrint, "设置角色%s, %d号属性%s\n", pCCha->GetLogName(), sAttrIndex, szValue);
	sprintf(szPrint, RES_STRING(GM_EXPAND_H_00010), pCCha->GetLogName(), sAttrIndex, szValue);
	g_pCLogObj->Log(szPrint);
	g_pCLogObj->Log("\n");
#endif
}
...
inline int lua_SetItemForgeParam(lua_State *pLS)
{
	...
	if (lType == 0)
		...
		pSItem->SetDBParam(enumITEMDBP_FORGE, (long)lua_tonumber(pLS, 3));
