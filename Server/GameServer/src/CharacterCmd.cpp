...
void CCharacter::Cmd_ItemAmphitheaterAsk(int ReID)
{
	...
	{
		SItemGrid* pItemGrid = NULL;
		pItemGrid = this->m_CKitbag.GetGridContByID(8 );
...
{T_B
    ...
    if(!m_CKitbag.IsPwdLocked())//是否设置了二次密码
    {
        Short i;
        Short sCapacity = m_CKitbag.GetCapacity();
        for(i = 0; i < sCapacity; i++)
        {
            if(m_CKitbag.GetID(i) == 2440)//密码锁的ID
            {
                SItemGrid Grid;
                Grid.sNum = 1;
                if(!m_CKitbag.Pop(&Grid, i))//取出密码锁
                {
                    //背包更新
                    SynKitbagNew(enumSYN_KITBAG_EQUIP);
...
{T_B
    ...
    if(cAuto == 0)
        SystemNotice("关闭道具栏自动锁定");
    else
        SystemNotice("打开道具栏自动锁定");
T_E}
...
{T_B
	...
	if(!ret)
	{
		SystemNotice("您已经在志愿者列表中了!");
	}
	else
	{
		SystemNotice("恭喜您已经光荣的成为志愿者了!");
	}
	return ret;
T_E}
...
{T_B
	...
	if(!ret)
	{
		SystemNotice("您不在志愿者列表中!");
	}
	else
	{
		SystemNotice("很遗憾的,您不再是志愿者了!");
	}
	return ret;
T_E}
