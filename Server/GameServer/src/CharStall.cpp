...
namespace mission
{
	#define MAX_STALL_MONEY 100000000 // 摆摊单个物品总价不可以超过1亿
	...
	void CStallSystem::StartStall( CCharacter& staller, RPACKET& packet )
	{
		...
		for( BYTE i = 0;i < pData->m_byNum; i++ )
		{
			...
			::SItemGrid*	grid	=	staller.m_CKitbag.GetGridContByID(	pData->m_Goods[i].byIndex	);

			if(	grid	&&	grid->dwDBID	)
			{
				pData->Free();
				staller.SystemNotice(	"道具%s已绑定，不能出售！",	pItem->szName	);
