...
{T_B
	...
	printf("Connectting database [%s : %s]......\n", g_Config.m_szDBIP, g_Config.databaseName_);
	...
	//	return FALSE;
	//}
	bool r = _connect.connect(g_Config.driverVersion_, g_Config.m_szDBIP, g_Config.databaseName_, g_Config.m_szDBUsr, pswd.c_str(), err_info);
    if(!r)
    {
		LG("gamedb", "msgDatabase [%s] Connect Failed!, ERROR REPORT[%d]", g_Config.databaseName_, err_info.c_str() );
        return FALSE;
    }
