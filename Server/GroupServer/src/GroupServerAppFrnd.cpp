	...
	if(l_inviter && l_inviter->m_currcha >=0 && l_inviter.m_chaid ==l_inviter->m_chaid[l_inviter->m_currcha])
	{
		...
		}else
		{
			...
			*/
			...
			if(m_tblfriends->GroupIsExsit(ply->m_chaid[ply->m_currcha],-1, gc_standard_group))
			{
				m_tblfriends->AddGroup(ply->m_chaid[ply->m_currcha],gc_standard_group);
			}
			// End
...
void GroupServerApp::CP_FRND_DEL_GROUP(Player *ply,DataSocket *datasock,RPacket &pk)
{
	...
	if(strcmp(l_grp, gc_standard_group) == 0)
	{
		ply->SendSysInfo("本分组不能删除！");
		return;
	}
...
void GroupServerApp::CP_FRND_CHANGE_GROUP(Player *ply,DataSocket *datasock,RPacket &pk)
{
	...
	if(strcmp(l_grp1, gc_standard_group) == 0)
	{
		ply->SendSysInfo("不能更改标准分组！");
		return;
	}

	if(strcmp(l_grp2, gc_standard_group) == 0)
	{
		ply->SendSysInfo("不能更改标准分组！");
		return;
	}
