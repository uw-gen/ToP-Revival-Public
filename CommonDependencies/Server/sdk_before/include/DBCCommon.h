...
public:
	...
	operator=(uLong mask)			{return (m_BitMask  = mask);}
...
//比较操作符定义
template<class T> operator <(const T &t1,const T &t2){return  (t2>t1);}
template<class T> operator<=(const T &t1,const T &t2){return !(t1>t2);}
template<class T> operator>=(const T &t1,const T &t2){return !(t2>t1);}
template<class T> operator!=(const T &t1,const T &t2){return !(t1==t2);}
