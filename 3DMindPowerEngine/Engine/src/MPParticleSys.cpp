...
//加入路径保存
	...
	if(_bModelRange)
	{
		...
		fwrite(_vecPointRange.data(), sizeof(D3DXVECTOR3), _wVecNum, t_pFile);
	}
...
bool	CMPPartSys::LoadFromFile(FILE* t_pFile, DWORD dwVersion)
{
	...
	if(dwVersion > 11)
	{
		if(_bModelRange)
		{
			...
			_vecPointRange.resize(_wVecNum);
			fread(_vecPointRange.data(), sizeof(D3DXVECTOR3), _wVecNum, t_pFile);
		}
	}
	if(dwVersion > 12)
	{
...
bool	CMPPartSys::LoadFromMemory(CMemoryBuf*	pbuf, DWORD dwVersion)
{
	...
	if(dwVersion > 11)
	{
		if(_bModelRange)
		{
			...
			_vecPointRange.resize(_wVecNum);
			pbuf->mread(_vecPointRange.data(), sizeof(D3DXVECTOR3), _wVecNum);
		}
	}
	if(dwVersion > 12)
	{
