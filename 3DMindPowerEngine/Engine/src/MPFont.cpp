...
bool CMPFont::TextToTexture( char c1, char c2, float & tX, float & tY )
{
	...
	if ( it == _vecBuf.end() )
	{
		...
		if ( it == _vecBuf.end() )
		{
			memset(_vecBuf.data(), 0, sizeof(WORD) * _Max);
...
void  CMPFont::RenderOptimize(int iIdx)
{
	...
	{
		_pDev->SetTexture( 0, _pTexFast->GetTex() );
		_pDev->GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLELIST, prc->iAshNum, prc->_vecAsh.data(), sizeof(FONT_VER));
	}
	if(prc->iHslNum)
	{	
		_pDev->SetTexture( 0, _pTex->GetTex() );
		_pDev->GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLELIST, prc->iHslNum, prc->_vecHsl.data(), sizeof(FONT_VER));
	}
	_pCEffectFile->End();
