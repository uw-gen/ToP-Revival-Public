...
VOID CLoginScene::FSCommand( char* szParam1, char* szParam2 )
{
	...
	else if( szParam2 && strstr( szParam2, "00" ) )
	{
		...
		sprintf( szFilename, "%s\\flash\\serverzone%d.txt", dir, m_iCurSelRegionIndex );
	...
	else if( szParam2 && stricmp( szParam2, "enter" ) == 0 )
	{
		char szFilename[_MAX_PATH] = { 0 };
		const char* dir = GetFlashDir( 2 );
		sprintf( szFilename, "%s\\flash\\serverzone%d.txt", dir, m_iCurSelRegionIndex );
...
void CLoginScene::_evtRegionLDBDown(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	for( int index = 0; index < NUM_REGIN_LIST; index ++ )
	{
		if( pkRegionList == lstRegion[index])
		{
			...
			sprintf( szFilename, "flash%d.swf", ServerGroupCnt );
			...
			if( GFlashPlayer )
			{
				...
				GFlashPlayer->GotoFrame( TotalFrames - 1, FALSE );
			}

			const char* dir = GetFlashDir( 2 );
			sprintf( szFlashXml, "%s\\flash\\%s", dir, "flash.xml" );
			sprintf( szFilename, "%s\\flash\\%s", dir, "text01.xml" );
			...
			for( int n = 0; n < MAX_SERVER_NUM; n ++ )
			{
				...
				sprintf( szPngfile, "%d%d%d%d.png", n + 1, n + 1, n + 1, n + 1 );
				Xml << newlinetag("data2") // child tag
					<< attr("pic") << szPngfile
			...
			sprintf( szFilename, "%s\\flash\\%s", dir, "111.xml" );
			ofstream Recordfile( szFilename );

			sprintf( szFilename, "%s\\flash\\serverzone%d.txt", dir, ReginIndex );
			FILE* fp = fopen( szFilename, "rb" );
			if( fp )
			{
				int ServerIndex = -1;
				...
				sprintf( szPngfile, "%d%d%d%d.png", ServerIndex + 1, ServerIndex + 1,
