...
void CItemCommand::AddHint( int x, int y )
{
    ...
    if( _pItem->sType>=1 && _pItem->sType<=10 )          // 武器:剑,巨剑,弓,火绳枪,刀,拳套,匕首,钱袋,短棒,锤子
    {
		...
		if ( CItemRecord::IsVaildFusionID(_pItem) && _ItemData.GetFusionItemID() > 0 )// modify by ning.yan  20080821
		{
			sprintf( buf, RES_STRING(CL_LANGUAGE_MATCH_627), _ItemData.GetItemLevel() * 2 + 80 );	// 发挥度定义 0级-80% 1级-82% ...
			PushHint( buf, GENERIC_COLOR );
		}
		...
		//	_PushValue( "命中率(%+d)", ITEMATTR_VAL_HIT, item );
		//	break;
		//case 2:	// 巨剑
	...
	{
        PushHint( GetName(), COLOR_WHITE, 5, 1 ); // 名字
  ...
  //      AddHintHeight();
		...
		{
			sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_862), _ItemData.sEndure[0] * 10 - 1000, _ItemData.sEndure[1] * 10 - 1000);// 具体计算方式询问策划
			PushHint( buf, COLOR_WHITE, 5, 1 );
	...
	else if( _pItem->sType == 46 )	// 勇者之证
	{
		...
		sprintf( buf, "%s:%d", RES_STRING(CL_LANGUAGE_MATCH_848), _ItemData.sEnergy[1] );	// 乱斗点数
		PushHint( buf, GENERIC_COLOR );
...
//	else if( enumItemTypePet==_pItem->sType )
	...
	else if( _pItem->sType == 65 )	// 黑龙祭坛  add by Philip.Wu  2006-06-19
	{
		...
		else if(_pItem->lID == 2954)	// 特殊道具处理（死亡证明）
		{
			...
			sprintf( buf, "%s: %d", RES_STRING(CL_LANGUAGE_MATCH_933), nCount );	// "死亡次数"
			PushHint( buf );
    ...
    else if(_pItem->sType == 70)
    {
		...
		{
			sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_909), item.sInstAttr[ITEMATTR_VAL_STR]); // "情缘编码：%d"
	        PushHint(buf, GENERIC_COLOR);
        ...
        if(_pItem->lID != 2236)
        {
            ...
            sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_897), _ItemData.sEnergy[0]); // "工具经验：%i"
            PushHint(buf, GENERIC_COLOR);
        }
	...
	else if( _pItem->sType==71 && _pItem->lID == 3289 )   //学生证处理
    {
		...
		{
			sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_949), arShowName[nLevel]);	// "学历:%s"
			PushHint(buf, COLOR_WHITE, 5, 1 );
		}

		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_950), item.sEndure[0], item.sEndure[1]);	// "学分(%d/%d)"
		PushHint(buf, COLOR_WHITE, 5, 1 );

		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_951), item.sEnergy[0] * 1000, item.sEnergy[1] * 1000);	// "经验(%d/%d)"
		PushHint(buf, COLOR_WHITE, 5, 1 );
	...
	else if( _pItem->sType== 75 )          // 彩票
	{
		...
		{
			//sprintf( buf, "期号：%03d", item.sInstAttr[ITEMATTR_VAL_STR] % 1000 );
			sprintf( buf, RES_STRING(CL_UIITEMCOMMAND_CPP_00006), item.sInstAttr[ITEMATTR_VAL_STR] % 1000 );
			PushHint( buf, GENERIC_COLOR );
			
			//sprintf( buf, "填写日期：%02d号%02d点", (short)floor( (float)item.sInstAttr[ITEMATTR_VAL_AGI] / 100),  item.sInstAttr[ITEMATTR_VAL_AGI] % 100);
			...
			//sprintf( buf, "彩球号：%c %c %c %c %c %c", c1, c2, c3, c4, c5, c6);
