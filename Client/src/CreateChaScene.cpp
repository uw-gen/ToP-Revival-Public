...
const char* CCreateChaScene::GetRaceDescription(uint8_t position)
{
    switch (position)
    {
    case 0:
...
const char* CCreateChaScene::GetCityName(uint8_t position)
{
    switch (position) {
    case 0:
...
const char* CCreateChaScene::GetCityDescription(uint8_t position)
{
    switch (position) {
    case 0:
                                            ...
                                            int x, int y, DWORD dwKey)
{
	...
	if (frmChaCity->GetIsShow())
	{
		...
		if (CITY_BY_INDEX<=index && index<=CITY_BL_INDEX)
		{
			...
			g_stUIBox.ShowSelectBox(_SelectCity, GetCityDescription(index-1), true);
		}
	}
...
void CCreateChaScene::InitChaFoundFrm()
{
    ...
    memChaDescribe->SetCaption(GetRaceDescription(m_nSelChaIndex));
...
void CCreateChaScene::SendChaToServ()
{
	...
	{
		CS_NewCha( edtName->GetCaption(), GetCityName(m_nCurCityIndex), part );
		CGameApp::Waiting();
	}
