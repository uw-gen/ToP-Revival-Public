...
long CSceneObjFile::Init(_TCHAR *ptcsFileName, bool bSilence)
{
	...
	if (m_SFileHead.lVersion != OBJ_FILE_VER600)
	{
		if (!bSilence)
		{
			_stprintf(tcsPrint, _TEXT("%s %s"), ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_340)));
			MessageBox(NULL, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
		}
		lRet = 0;
		goto end;
	...
	if (m_fRdWr == NULL)
	{
		if (!bSilence)
		{
			_stprintf(tcsPrint, _TEXT("%s %s"), ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_339)));
			MessageBox(NULL, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
		}
		lRet = 0;
		goto end;
	...
	if (m_fAppend == NULL)
	{
		if (!bSilence)
		{
			_stprintf(tcsPrint, _TEXT("%s %s"), ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_359)));
			MessageBox(NULL, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
		}
		lRet = 0;
		goto end;
...
long CSceneObjFile::ConvertObjFileVer(_TCHAR *ptcsFileName, bool bBackUp) // 从版本500转换到600(物件的绝对坐标转换成相对section的相对坐标)
{
	...
	_stprintf(tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_360)), ptcsFileName);
	...
	if (lRet == 2)
	{
		_stprintf(tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_362)), ptcsFileName, tcsBackUpName);
...
long CSceneObjFile::TrimDirectory(_TCHAR *ptcsDirectory, bool bBackUp)
{
	...
	if (bBackUp)
		_stprintf(tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_366)), ptcsDirectory);
	else
		_stprintf(tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_367)), ptcsDirectory);
	...
	if (fRecord == NULL)
		return 0;
	_ftime(&tTimeBuffer);
	_ftprintf(fRecord, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_368)), _tctime(&tTimeBuffer.time));
